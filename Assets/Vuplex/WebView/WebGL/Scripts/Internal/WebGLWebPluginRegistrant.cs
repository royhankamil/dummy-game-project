...
namespace Vuplex.WebView.Internal {
    /// <summary>
    /// Registers the WebGL web plugin.
    /// </summary>
    /// <remarks>
    ...
    /// using the `UNITY_EDITOR` directive, which the WebGLWebPlugin class uses.
    /// This class works around that issue by using the `UNITY_EDITOR` directive *inside*
    /// the method instead of outside.
    /// </remarks>
    class WebGLWebPluginRegistrant {

        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.BeforeSceneLoad)]
        static void _registerPlugin() {

            #if UNITY_WEBGL && !UNITY_EDITOR
                WebPluginFactory.RegisterWebGLPlugin(WebGLWebPlugin.Instance);
            #endif
        }
    }
}
